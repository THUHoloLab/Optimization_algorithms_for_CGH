...
%         amp=sqrt(E.*mask/sum(mask(:)));
    ...
    Am=sqrt(E*(abs(wavefront).^2)./sum(sum(abs(wavefront).^2)));
    wavefront= Am.*exp(1i*angle(wavefront));
    objectField=opt.source.*wavefront;
    for s=1:slice
...
for s=1:slice
imwrite(I(:,:,s),[num2str(s),'sequential_3D_CH.bmp']);
end
