...
slm.window = ones(slm.Nx, slm.Ny,5);
slm.window = padarray(slm.window,[slm.Nx/2,slm.Ny/2,0]);
...
pha=2*pi*rand(opt.Nx,opt.Ny,slice);
%% optimization
...
lb = -inf(opt.Nx*opt.Ny*slice, 1);
ub = inf(opt.Nx*opt.Ny*slice, 1);
times=10;
RMSE=zeros(times,1);
I=zeros(slm.Nx, slm.Ny,slice);
f = @(x)Gradient_L2_3D_AS_CH(x, opt.source, opt.Nx, opt.Ny, Masks, HTrans);
for i=2:times
    [pha, loss] = fmincon(f,pha,[],[],[],[],lb,ub,[],matlab_options);
%% show result
    for s=1:slice
    objectField = sqrt(Masks(:,:,s)).*exp(1i.*pha(:,:,s));
...
%     wavefront= Am.*exp(1i*angle(imagez));
    imagez = fftshift(fft2(fftshift(imagez))) .* conj(HTrans(:,:,s));
    imagez = ifftshift(ifft2(ifftshift(imagez))); 
    amp=abs(imagez);
...
for s=1:slice
imwrite(I(:,:,s),[num2str(s),'super_3D_POH.bmp']);
end
